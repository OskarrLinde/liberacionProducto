...
namespace LiberacionProductoWeb.Models.DataBaseModels
{
    public class ConditioningOrder : Entity, ICloneable
    {
        ...
        public Guid GroupIdChecklist { get; set; }
        public string CloseOrderUserName { get; set; }
        public DateTime? CloseOrderUserDate { get; set; }
        public string CloseOrderUserNote { get; set; }
        ...
        public ConditioningOrder(int id, DateTime? endDate, string reasonReject, bool? isReleased,
            ...
            bool? inCompliance, string delegateUser, ProductionOrder productionOrder, Guid groupId, Guid groupIdChecklist)
        {
            this.Id = id;
            ...
            this.GroupIdChecklist = groupIdChecklist;
        }
            ...
            Guid groupIdChecklist)
        {
            var entity = new ConditioningOrder
            {
                ...
                GroupId = groupId,
                GroupIdChecklist = groupIdChecklist
            };
            return entity;
        }
                ...
                if (old.EndDate != current.EndDate)
                {
                    if (old.State != current.State)
                    {
                        auditList.Add(new ReportAuditTrail
                        {
                            ...
                            Date = DateTime.Now,
                            Detail = "Tabla 7: Cierre de expediente de lote - Campo - Fecha término",
                ...
                if (old.IsReleased != current.IsReleased)
                {
                    if (old.State != current.State)
                    {
                        auditList.Add(new ReportAuditTrail
                        {
                            ...
                            Controller = "ConditioningOrder",
                            Date = DateTime.Now,
                            Detail = "Tabla 7: Cierre de expediente de lote - Campo - Liberación",
                            Funcionality = "Orden de acondicionamiento",
                ...
                if (old.ReleasedNotes != current.ReleasedNotes)
                {
                    if (current.State == ConditioningOrderStatus.Released.Value)
                    {
                        auditList.Add(new ReportAuditTrail
                        {
                            ...
                            Controller = "ConditioningOrder",
                            Date = DateTime.Now,
                            Detail = "Tabla 7: Cierre de expediente de lote- Campo - Observaciones",
                            Funcionality = "Orden de acondicionamiento",
            ...
            if (old != null)
            {
                if (current.State == ConditioningOrderStatus.Released.Value)
                {
                    auditList.Add(new ReportAuditTrail
                    {
                        ...
                        Controller = "ConditioningOrder",
                        Date = DateTime.Now,
                        Detail = "Tabla 7: Cierre de expediente de lote - Campo - usuario firma",
                        Funcionality = "Orden de acondicionamiento",
        ...
        public object Clone()
        {
            return new ConditioningOrder
            {
                ...
                FooterCertificateId = this.FooterCertificateId,
                CertificateId = this.CertificateId,
                GroupId = this.GroupId,
                GroupIdChecklist = this.GroupIdChecklist
            };
        }
    }
