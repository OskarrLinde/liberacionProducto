...
namespace LiberacionProductoWeb.Models.DataBaseModels
{
    public class ComplementoTanque : Entity, ICloneable
    {
        ...
        public string InformeMayor { get; set; }
        [Column(TypeName = "varchar(500)")]
        public string InformeMenor { get; set; }
        //-----------------------------
        //Relationships
        ...
        public override IEnumerable<ReportAuditTrail> AuditTrailComparison(Entity objectToCompare, Entity objectToCompareOld = null, string DistribuitionBatch = null)
        {
            ...
            if (old.InformeMayor != current.InformeMayor)
            {
                auditList.Add(new ReportAuditTrail
                {
                    Action = "Modificación",
                    Controller = "RapTanques",
                    Date = DateTime.Now,
                    Detail = "Complemento Rap tanques- Campo - Informe de Desviación Mayor",
                    Funcionality = "Rap tanque complemento",
                    PreviousValue = old.InformeMayor,
                    NewValue = current.InformeMenor,
                    Method = "UpdateAsync",
                    Plant = current.productionOrder.PlantId,
                    Product = current.productionOrder.ProductId,
                    User = current.productionOrder.CreatedBy,
            ...
            if (old.InformeMenor != current.InformeMenor)
            {
                auditList.Add(new ReportAuditTrail
                {
                    Action = "Modificación",
                    Controller = "RapTanques",
                    Date = DateTime.Now,
                    Detail = "Complemento Rap tanques- Campo - Informe de Desviación Menor",
                    Funcionality = "Rap tanque complemento",
                    PreviousValue = old.InformeMenor,
                    NewValue = current.InformeMenor,
                    Method = "UpdateAsync",
                    Plant = current.productionOrder.PlantId,
                    Product = current.productionOrder.ProductId,
