    ...
    public class RAPPipasController : Controller
    {
        ...
        String SelectedPharmaceuticalFormFilter, DateTime? StartDate, DateTime? EndDate)
        {
            ...
            try
            {
                ...
                pipaModel.Rechazados = pipaModel.ListRAPPipas.Count(s => s.Aprobado == "No").ToString();
                // "Total = Aprobados + Rechazados"
                pipaModel.Total = pipaModel.ListRAPPipas.Count(x => !string.IsNullOrEmpty(x.Aprobado)).ToString();
            ...
            try
            {
                ...
                foreach (var obj in mapped)
                {
                    ...
                    else
                    {
                        ...
                        entity.InformeMayor = obj.InformeMayor?.Trim() ?? "NA";
                        entity.InformeMenor = obj.InformeMenor?.Trim() ?? "NA";
