...
namespace LiberacionProductoWeb.Controllers
{
    ...
    public class QuerysFilesController : Controller
    {
        ...
        public QuerysFilesController(UserManager<ApplicationUser> userManager,
            IPrincipalService principalService,
            IStringLocalizer<Resource> resource,
            ILogger<QuerysFilesController> logger,
        ...
        [HttpGet]
        [Authorize(SecurityConstants.CONSULTAR_EXPEDIENTE_DE_LOTE)]
        public async Task<IActionResult> DetailDB(int Id, [FromServices] IProductionOrderRepository productionOrderRepository, [FromServices] IConditioningOrderRepository conditioningOrderRepository)
        {
            List<DetailDistributionBatch> model = new List<DetailDistributionBatch>();
            try
            {
                var productionOrder = await productionOrderRepository.FirstOrDefaultAsync(Id);
                if (productionOrder is null)
                {
                    throw new Exception($"No se encontro la Orden de producción con Id: {Id}");
                }
                var conditioning = await conditioningOrderRepository.FirstOrDefaultAsync(x => x.ProductionOrderId == productionOrder.Id);
                if (conditioning == null)
                {
                    throw new Exception($"No se encontro la Orden de acondiconamiento con ProductionOrderId: {Id}");
                }
                model = await _principalService.GetDetailDB(Id, conditioning, productionOrder);
            }
            catch (Exception ex)
            {
                //_logger.LogInformation("Ocurrio un error en principal " + ex);
        ...
        public async Task<IActionResult> Summary(int id)
        {
            ...
            foreach (var item in model.ProductionOrder.LayoutLeyends)
            {
                item.Text = item.Text.Replace("{CaseTag:Location}", model.ProductionOrder.Location)
                               .Replace("{CaseTag:ProductName}", model.ProductionOrder.ProductName)
                               .Replace("{CaseTag:ProductCode}", model.ProductionOrder.ProductCode)
                               .Replace("{CaseTag:SelectedTankFilter}", model.ProductionOrder.SelectedTankFilter);
            }
            foreach (var item in model.ConditioningOrder.LayoutLeyends)
            {
                item.Text = item.Text.Replace("{CaseTag:Location}", model.ConditioningOrder.Plant)
                           .Replace("{CaseTag:ProductName}", model.ConditioningOrder.Product)
                           .Replace("{CaseTag:ProductCode}", model.ConditioningOrder.ProductName)
                           .Replace("{CaseTag:SelectedTankFilter}", model.ConditioningOrder.Tank);
            }
            try
            {
        ...
        private async Task<List<SelectListItem>> GetTanksItemsAsync(string plantId, string productId)
        {
            ...
            var response = tanks.Where(x => tanksFiltered.Contains(x.Text)).ToList();

            return response;
        }

        [HttpGet]
        public async Task<IActionResult> ExportSummary(int Id, [FromServices] ILeyendsCertificateHistoryRepository leyendsCertificateHistoryRepository,
            [FromServices] ILeyendsFooterCertificateHistoryRepository leyendsFooterCertificateHistoryRepository,
            [FromServices] IConditioningOrderRepository conditioningOrderRepository)
        {
            //ProductionOrder
            ...
            try
            {
                ...
                var leyends = await _productionOrderService.GenerateLeyends(model);
                foreach (var item in leyends)
                {
                    item.Text = item.Text.Replace("{CaseTag:Location}", model.Location)
                                   .Replace("{CaseTag:ProductName}", model.ProductName)
                                   .Replace("{CaseTag:ProductCode}", model.ProductCode)
                                   .Replace("{CaseTag:SelectedTankFilter}", model.SelectedTankFilter);

                }
                ///Product Plant 
                ///title
                XRLabel titleOne = _rptOP.FindControl("xrLabel3", true) as XRLabel;
                titleOne.Text = leyends.Where(x => x.Step.Equals(1)).Select(x => x.Title).FirstOrDefault();
                XRRichText Leyend = _rptOP.FindControl("xrLabel4", true) as XRRichText;
                var leyend = string.Empty;
                leyend = leyends.Where(x => x.Step.Equals(1)).Select(x => x.Text).FirstOrDefault();
                Leyend.Html = leyend;

                //pipeLine
                XRLabel titleTwo = _rptOP.FindControl("xrLabel27", true) as XRLabel;
                titleTwo.Text = leyends.Where(x => x.Step.Equals(2)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendPl = _rptOP.FindControl("xrLabel26", true) as XRRichText;
                leyend = string.Empty;

                leyend = leyends.Where(x => x.Step.Equals(2)).Select(x => x.Text).FirstOrDefault();
                LeyendPl.Html = leyend;
                ///lote prod table 4
                leyend = string.Empty;
                XRLabel titleThree = _rptOP.FindControl("xrLabel61", true) as XRLabel;
                titleThree.Text = leyends.Where(x => x.Step.Equals(3)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendLote = _rptOP.FindControl("xrLabel60", true) as XRRichText;
                LeyendLote.Html = leyends.Where(x => x.Step.Equals(3)).Select(x => x.Text).FirstOrDefault();
                //Change text CO2
                XRLabel titleFor = stp6.FindControl("xrLabel27", true) as XRLabel;
                titleFor.Text = leyends.Where(x => x.Step.Equals(4)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendQA = stp6.FindControl("xrRichText1", true) as XRRichText;
                LeyendQA.Html = leyends.Where(x => x.Step.Equals(4)).Select(x => x.Text).FirstOrDefault();
                //batchDetails
                XRLabel titleFive = stp7.FindControl("xrLabel27", true) as XRLabel;
                titleFive.Text = leyends.Where(x => x.Step.Equals(5)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendBatch = stp7.FindControl("xrLabel3", true) as XRRichText;
                leyend = string.Empty;
                leyend = leyends.Where(x => x.Step.Equals(5)).Select(x => x.Text).FirstOrDefault();
                LeyendBatch.Html = leyend;

                //ReleaseProduct
                XRLabel titleSix = stp8.FindControl("xrLabel27", true) as XRLabel;
                titleSix.Text = leyends.Where(x => x.Step.Equals(6)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendRelease = stp8.FindControl("xrLabel6", true) as XRRichText;
                leyend = string.Empty;
                ...
                leyend = leyends.Where(x => x.Step.Equals(6)).Select(x => x.Text).FirstOrDefault();
                LeyendRelease.Html = leyend;

                XRLabel titleSeven = stp8.FindControl("label2", true) as XRLabel;
                titleSeven.Text = leyends.Where(x => x.Step.Equals(7)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendReleaseOther = stp8.FindControl("xrRelease", true) as XRRichText;
                leyend = string.Empty;
                leyend = leyends.Where(x => x.Step.Equals(7)).Select(x => x.Text).FirstOrDefault();
                LeyendReleaseOther.Html = leyend;
            ...
            try
            {
                var conditioningOrder = await conditioningOrderRepository.FirstOrDefaultAsync(x => x.ProductionOrderId == Id);
                modelOA = await _exportPDFService.GetModel(Id, 0, conditioningOrder, true);
                foreach (var item in modelOA.LayoutLeyends)
                {
                    item.Text = item.Text.Replace("{CaseTag:Location}", modelOA.Plant)
                                   .Replace("{CaseTag:ProductName}", modelOA.Product)
                                   .Replace("{CaseTag:ProductCode}", modelOA.ProductName)
                                   .Replace("{CaseTag:SelectedTankFilter}", modelOA.Tank);
                }
                if (modelOA == null)
                {
                    return BadRequest();
                ...
                if ((modelOA != null && modelOA.IsReleased.HasValue && modelOA.IsReleased.Value))
                {
                    ...
                    TitleOne.Text = modelOA.LayoutLeyends.Where(x => x.Step.Equals(1)).Select(x => x.Title).FirstOrDefault();
                    XRRichText LeyendOne = _rptOA.FindControl("xrTextOne", true) as XRRichText;
                    leyend = modelOA.LayoutLeyends.Where(x => x.Step.Equals(1)).Select(x => x.Text).FirstOrDefault();
                    ...
                    TitleTwo.Text = modelOA.LayoutLeyends.Where(x => x.Step.Equals(2)).Select(x => x.Title).FirstOrDefault();
                    XRRichText LeyendTwo = _rptOA.FindControl("xrTextTwo", true) as XRRichText;
                    leyend = string.Empty;
                    leyend = modelOA.LayoutLeyends.Where(x => x.Step.Equals(2)).Select(x => x.Text).FirstOrDefault();
                    LeyendTwo.Html = leyend;

                    XRLabel TitleThree = _rptOA.FindControl("label25", true) as XRLabel;
                    TitleThree.Text = modelOA.LayoutLeyends.Where(x => x.Step.Equals(3)).Select(x => x.Title).FirstOrDefault();

                    XRRichText LeyendThree = _rptOA.FindControl("xrTextThree", true) as XRRichText;
                    leyend = string.Empty;
                    leyend = modelOA.LayoutLeyends.Where(x => x.Step.Equals(3)).Select(x => x.Text).FirstOrDefault();
                    LeyendThree.Html = leyend;

                    XRLabel TitleFor = _rptOA.FindControl("label49", true) as XRLabel;
                    TitleFor.Text = modelOA.LayoutLeyends.Where(x => x.Step.Equals(4)).Select(x => x.Title).FirstOrDefault();
                    XRRichText LeyendFor = _rptOA.FindControl("label50", true) as XRRichText;
                    leyend = string.Empty;
                    leyend = modelOA.LayoutLeyends.Where(x => x.Step.Equals(4)).Select(x => x.Text).FirstOrDefault();
        ...
        [HttpGet]
        public async Task<IActionResult> ExportDistribuitionBatchClients(int Id, [FromServices] IProductionOrderRepository productionOrderRepository, [FromServices] IConditioningOrderRepository conditioningOrderRepository)
        {
            ...
            try
            {
                ...
                basicInfo = await _conditioningOrderService.GetBasicInfoConditioningOrderByOp(Id, conditioning, productionOrder);
                model.detailDB = await _principalService.GetDetailDB(Id, conditioning, productionOrder);
