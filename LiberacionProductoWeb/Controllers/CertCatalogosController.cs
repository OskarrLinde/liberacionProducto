...
namespace LiberacionProductoWeb.Controllers
{
    public class CertCatalogosController : Controller
    {
        ...
        public IActionResult Analizador(bool showAll = false)
        {
            ...
            var producto = getProductos().Where(x => x.iD_STATUS == 1).ToList();
        ...
        public JsonResult GetAnalizadorHTMLTagsById(string Id)
        {
            ...
            var analizador = getAnalizador();
            var products = getProductos().Where(x => x.iD_STATUS == 1).ToList(); ;
            var _url = _catalogCertificate.urlCatalogs + "Analizador";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<Analizador, Analizador>(_url + "/" + Id);
            JsonDeserializer deserial = new JsonDeserializer();
        ...
        public async Task<IActionResult> AnalizadorParametros(bool showAll = false)
        {
            ...
            sourceSupply = sourceSupply.Where(x => x.iD_STATUS == 1).ToList();
			approvedPlant = approvedPlant.Where(x => x.iD_STATUS == 1).ToList();
            ...
            if (!showAll)
                _analizadorParametrosVM.PlantasList = _analizadorParametrosVM.PlantasList.Where(x => x.iD_STATUS == 1).ToList();
            ...
            if (country != null)
            {
                _analizadorParametrosVM.PaisesList = country;
                _analizadorParametrosVM.PaisesFilter = country.ConvertAll(a =>
                {
                    return new SelectListItem()
                    {
                        Text = a.nombre,
            ...
            if (supplyType != null)
            {
                _analizadorParametrosVM.TipoSuministrosList = supplyType;
                _analizadorParametrosVM.TipoSuministrosFilter = supplyType.ConvertAll(a =>
                {
                    return new SelectListItem()
                    {
                        Text = a.descripcion,
            ...
            {
                _analizadorParametrosVM.FuenteSuministroList = sourceSupply;
                _analizadorParametrosVM.FuenteSuministroFilter = sourceSupply.ConvertAll(a =>
                {
                    return new SelectListItem()
                    {
                        Text = a.descripcion,
            ...
            {
                _analizadorParametrosVM.PlantaAprobadaList = approvedPlant;
                _analizadorParametrosVM.PlantaAprobadFilter = approvedPlant.ConvertAll(a =>
                {
                    return new SelectListItem()
                    {
                        Text = a.descripcion,
        ...
        public async Task<JsonResult> GetAnalizadorParametrosHTMLTagsById(string Id)
        {
            ...
            var analizador = getAnalizador().Where(x => x.iD_STATUS == 1).ToList();
            var parametro = getParametro().Where(x => x.iD_STATUS == 1).ToList();
            var metodo = getMetodo().Where(x => x.iD_STATUS == 1).ToList();
        ...
        public async Task<JsonResult> GetAnalizadorProductoHTMLTagsById(string Id)
        {
            ...
            var productos = getProductos().Where(x => x.iD_STATUS==1).ToList();
        ...
        public async Task<JsonResult> GetClientesHTMLTagsById(string Id)
        {
            ...
            tanks = tanks.Where(x => x.iD_STATUS == 1).ToList();
			var products = getProductos().Where(x => x.iD_STATUS == 1).ToList();
            var grade = getGrado().Where(x => x.iD_STATUS == 1).ToList();
            var typeCertificate = getTipoCertificado().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "Cliente";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<Clientes, Clientes>(_url + "/" + Id);
            JsonDeserializer deserial = new JsonDeserializer();
        ...
        public JsonResult GetEspecificacionDetalleHTMLTagsById(string Id)
        {
            ...
            var parameter = getParametro().Where(x => x.iD_STATUS == 1).ToList();
            var unitMeasurement = getUnidadMedidas().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "EspecificacionDetalle";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<EspecificacionDetalle, EspecificacionDetalle>(_url + "/" + Id);
            JsonDeserializer deserial = new JsonDeserializer();
        ...
        public IActionResult EspecificacionMaster(bool showAll = false)
        {
            ...
            lstSpecificationMasterExportExcels = (from masters in especificacionMasterExportExcel
                                                  ...
                                                  on masters.iD_ESPECIFICACION equals details.iD_ESPECIFICACION
                                                  into joinedList
            ...
            plantaProducto = plantaProducto.Where(x => x.iD_STATUS == 1).ToList();

            var groupPlantaProducto = plantaProducto
                .GroupBy(p => p.iD_PLANTA)
                .Select(g => new
                {
                    iD_PLANTA = g.Key,
                    Productos = g.ToList()
            ...
            _especificacionMasterVM.PlantasFilter = plantas.Join(
                groupPlantaProducto,
                planta => planta.iD_PLANTA,
                grupo => grupo.iD_PLANTA,
                (planta, grupo) => new SelectListItem
                {
                    Text = $"{planta.descripcion}",
                    Value = planta.iD_PLANTA.ToString()
                }
            ).ToList();
        ...
        {
            String response = String.Empty;
            var plants = getPlantas().Where(x => x.iD_STATUS == 1).ToList();
            var products = getProductos().Where(x => x.iD_STATUS == 1).ToList();
			var _url = _catalogCertificate.urlCatalogs + "EspecificacionMaster";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<EspecificacionMaster, EspecificacionMaster>(_url + "/" + Id);
			...
			var plantaProducto = getPlantaProducto().Where(x => x.iD_STATUS == 1 && x.iD_PLANTA == Convert.ToInt32(entity.iD_PLANTA)).ToList();

			var filteredProducts = products.Join(
				plantaProducto,
				products => products.iD_PRODUCTO,
				plantaProducto => plantaProducto.iD_PRODUCTO,
				(products, plantaProducto) => new Productos
				{
					iD_PRODUCTO = Convert.ToInt32(products.iD_PRODUCTO),
			...
			if (entity != null)
            {
                ...
                var plantaTag = "<select id='iD_PLANTA' onChange='onChangePlanta(this);' class='form-control'>";
        ...
        public JsonResult GetSpecificationMasterById(string Id)
        {
            ...
            var especificacionDetalle = getEspecificacionDetalle();
            var parametro = getParametro().Where(x => x.iD_STATUS == 1).ToList();
            var unidadMedida = getUnidadMedidas().Where(x => x.iD_STATUS == 1).ToList();
        ...
        {
            String response = String.Empty;
            var clients = getClientes().Where(x => x.status == 1).ToList();
            var tanks = await getTanque();
            tanks = tanks.Where(x => x.iD_STATUS == 1).ToList();
        ...
        public JsonResult GetGradoHTMLTagsById(string Id)
        {
            ...
            var grado = getGrado().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "Grado";
        ...
        {
            String response = String.Empty;
            var paises = getPaises().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "Paises";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<Paises, Paises>(_url + "/" + Id);
            JsonDeserializer deserial = new JsonDeserializer();
        ...
        public JsonResult GetParametroHTMLTagsById(string Id)
        {
            ...
            var unitMeasure = getUnidadMedidas().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "Parametro";
            RestGenerico _rest = new RestGenerico();
            var _result = _rest.getApi<Parametro, Parametro>(_url + "/" + Id);
            JsonDeserializer deserial = new JsonDeserializer();
        ...
        public JsonResult SaveOrEditPlanta([FromBody] DtoPlantas data)
        {
            try
            {
                ...
                planta.identificador = data.identificador ?? string.Empty;
        ...
        public async Task<JsonResult> GetPlantaHTMLTagsById(string Id)
        {
            String response = String.Empty;
            //var plants = getPlantas();
            var country = getPaises().Where(x => x.iD_STATUS == 1).ToList();
            var supplyType = getTipoSuministro().Where(x => x.iD_STATUS == 1).ToList(); ;
            var sourceSupply = await getFuenteSuministro();
            var approvedPlant = await getPlantaAprobada();
        ...
        public IActionResult PlantaProducto(bool showAll = false)
        {
            ...
            _plantaProductoVM.showAll = showAll;
            ...
            if (!showAll)
                _plantaProductoVM.PlantasList = _plantaProductoVM.PlantasList.Where(x => x.iD_STATUS == 1).ToList();
        ...
        [HttpGet]
        public JsonResult GetProductosByPlanta(int idPlanta)
        {
            var plantaProducto = getPlantaProducto().Where(x => x.iD_STATUS == 1 && x.iD_PLANTA == Convert.ToInt32(idPlanta)).ToList();
            var productos = getProductos().Where(x => x.iD_STATUS == 1).ToList();

            var filteredProducts = productos.Join(
                plantaProducto,
                productos => productos.iD_PRODUCTO,
                plantaProducto => plantaProducto.iD_PRODUCTO,
                (productos, plantaProducto) => new SelectListItem
                {
                    Text = $"{productos.nombre}",
                    Value = productos.iD_PRODUCTO.ToString()
                }
            ).ToList();
		...
		public IActionResult ProductoGrados(bool showAll = false)
        {
            ...
            if (!showAll)
                _productoGradoVM.ProductosList = _productoGradoVM.ProductosList.Where(x => x.iD_STATUS == 1).ToList();
        ...
        public IActionResult ProductoParametros(bool showAll = false)
        {
        ...
        {
            String response = String.Empty;
            var rol = getRol().Where(x => x.iD_STATUS == 1).ToList();
            var pais = getPaises().Where(x => x.iD_STATUS == 1).ToList();
        ...
        public async Task<JsonResult> GetTanqueHTMLTagsById(string Id)
        {
            ...
            var tanque = await getTanque();
            var plants = getPlantas().Where(x => x.iD_STATUS == 1).ToList();
            var product = getProductos().Where(x => x.iD_STATUS == 1).ToList();
            var grados = getGrado().Where(x => x.iD_STATUS == 1).ToList();
        ...
        public JsonResult SaveOrEditTransporte([FromBody] DtoTransporte data)
        {
            try
            {
                if (data == null)
                {
                    throw new ArgumentNullException(nameof(data), "La información enviada es nula.");
                }

                Transporte transporte = MapDtoToTransporte(data);

                if (string.IsNullOrEmpty(data.iD_TRANSPORTE))
                {
                    return HandleCreateTransporte(transporte);
                }
                else
                {
                    return HandleEditTransporte(transporte);
                }
            }
            catch (Exception ex)
            {
        ...
        public JsonResult DeleteTransporte(string id)
        {
            try
            {
                ...
                var url = _catalogCertificate.urlCatalogs + "Transporte/" + id;
                var restClient = new RestGenerico();
        ...
        public JsonResult GetTransporteHTMLTagsById(string Id)
        {
            ...
            var typeTransport = getTipoTransporte().Where(x => x.iD_STATUS == 1).ToList();
            var country = getPaises().Where(x => x.iD_STATUS == 1).ToList();
            var producto = getProductos().Where(x => x.iD_STATUS == 1).ToList();
            var statusPipa = getStatusPipa().Where(x => x.iD_STATUS == 1).ToList();
            var _url = _catalogCertificate.urlCatalogs + "Transporte";
            RestGenerico _rest = new RestGenerico();
        ...
        private Transporte MapDtoToTransporte(DtoTransporte data)
        {
            return new Transporte
            {
                iD_STATUS = ParseStatus(data.iD_STATUS),
                iD_TRANSPORTE = string.IsNullOrEmpty(data.iD_TRANSPORTE) ? (int?)null : int.Parse(data.iD_TRANSPORTE),
                iD_TIPO_TRANSPORTE = int.Parse(data.iD_TIPO_TRANSPORTE),
                iD_PAIS = data.iD_PAIS,
                iD_PRODUCTO = int.Parse(data.iD_PRODUCTO),
                descripcion = data.descripcion,
                clavE_PALS = data.clavE_PALS,
                usR_ALTA = 1,
                usR_MODIFICA = 1,
                iD_GRADO = "1"
            };
        ...
        private JsonResult HandleCreateTransporte(Transporte transporte)
        {
            var url = _catalogCertificate.urlCatalogs + "Transporte";
            var restClient = new RestGenerico();

            var result = restClient.postApi<Transporte, Transporte>(url, transporte);
            var response = DeserializeResponse(result);

            if (response["error"].Trim() == "False")
            {
                restClient.getApi<Transporte, Transporte>(url);
                return Json(new { Result = "Ok" });
            }
        ...
        private JsonResult HandleEditTransporte(Transporte transporte)
        {
            var url = _catalogCertificate.urlCatalogs + "Transporte/" + transporte.iD_TRANSPORTE;
            var restClient = new RestGenerico();
            ...
            throw new Exception("Los datos ya existen, no es posible modificar el catálogo");
        }
        #endregion
        ...
        private T DeserializeResponse<T>(IRestResponse response, string key = null)
        {
            ...
            {
                throw new ArgumentNullException(nameof(response), "La respuesta proporcionada es nula o vacía.");
            }

            var deserializer = new JsonDeserializer();
            var jsonObject = DeserializeResponse(response);

            if (key != null && jsonObject.ContainsKey(key))
            {
                return JsonConvert.DeserializeObject<T>(jsonObject[key]);
            }

            return JsonConvert.DeserializeObject<T>(response.Content);
        }

        private Dictionary<string, string> DeserializeResponse(IRestResponse jsonResponse)
        {
            var deserializer = new JsonDeserializer();
