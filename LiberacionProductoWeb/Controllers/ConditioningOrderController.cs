...
namespace LiberacionProductoWeb.Controllers
{
    ...
    public class ConditioningOrderController : Controller
    {
        ...
        public ConditioningOrderController(ILogger<ConditioningOrderController> logger,
        ...
        public async Task<IActionResult> Index(int Id, int IdOP, int preview = 0)
        {
            var model = new ConditioningOrderViewModel();
            try
            {
                ConditioningOrder conditioningOrder = null;
                //if (IdOP > 0 && Id == 0)
                //{
                ...
                //else if (Id > 0)
                //{
                //    var conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(Id);
                //    model = await _conditioningOrderService.GetByIdAsync(Id, conditioningOrder, preview: preview);
                //}
                if (IdOP > 0 && Id == 0)
                {
                    conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(x => x.ProductionOrderId == IdOP);
                }
                else if (Id > 0)
                {
                    conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(Id);
                ...
                foreach (var item in model.LayoutLeyends)
                {
                    item.Text = item.Text.Replace("{CaseTag:Location}", model.Plant)
                                   .Replace("{CaseTag:ProductName}", model.Product)
                                   .Replace("{CaseTag:ProductCode}", model.ProductName)
                                   .Replace("{CaseTag:SelectedTankFilter}", model.Tank);
                }
                if (preview != 0)
                    model.PreviewText = true;
                return View(model);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> New(int Id, int IdOP)
        {
            ...
            try
            {
                ...
                //{
                //    var conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(Id);
                //    model = await _conditioningOrderService.GetByIdAsync(Id, conditioningOrder);
                //}
                ConditioningOrder conditioningOrder = null;
                if (IdOP > 0 && Id == 0)
                {
        ...
        public async Task<IActionResult> GetEquipment(string tournumberList, int id, DateTime? datePipelineClearance, bool refresh = false)
        {
            ...
            if (id > 0)
            {
                ...
                {
                    var conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(id);
                    if (conditioningOrder is null)
                    {
                        _logger.LogError($"No se encontró la Orden de acondicionamiento: {id} en GetEquipment");
                        return Json(new { Result = "NotFound", Message = "Datos inválidos." });
                    }
                    model = await _conditioningOrderService.GetByIdAsync(id, conditioningOrder);
                    ...
                    //TODO: Mejorar flujo de este proceso, se agregan consulta a un nivel superior para quitar consultas dentro del for.
                    //var conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(id);

                    var productionOrder = await _productionOrderRepository.FirstOrDefaultAsync(conditioningOrder.ProductionOrderId);
                    if (productionOrder is null)
                    {
                        _logger.LogError($"No se encontró la Orden de producción: {conditioningOrder.ProductionOrderId} en GetEquipment");
                        return Json(new { Result = "NotFound", Message = "Datos inválidos." });
                    }
                    if (!string.IsNullOrEmpty(tournumberList))
                    {
                        foreach (var tournumber in tournumberList.Split(','))
                        {
                            if (refresh)
                            {
                                var equipamentProcessesList = await _conditioningOrderService.GetTable4(tournumber, id, datePipelineClearance, conditioningOrder, productionOrder);
                    ...
                    // Get info from general catalog
                    //var productionOrder = await _productionOrderRepository.GetByIdAsync(model.ProductionOrderId);
                    var generalCatalogFilter = await _generalCatalogRepository.GetAllAsync();
                    foreach (var item in generalCatalogFilter)
                    {
                        if (item.PlantId == productionOrder.PlantId)
                        {
                            generalCatalogFilter = (from r in generalCatalogFilter where productionOrder.PlantId.Contains(r.PlantId) select r).ToList();
                        }
                        if (item.ProductId == productionOrder.ProductId)
                        {
                            generalCatalogFilter = (from r in generalCatalogFilter where productionOrder.ProductId.Contains(r.ProductId) select r).ToList();
                        }
                        if (item.TankId == productionOrder.TankId)
                        {
                            generalCatalogFilter = (from r in generalCatalogFilter where productionOrder.TankId.Contains(r.TankId) select r).ToList();
                        }
                    }

                    var bayAreaIndex = 0;
                    model.BayAreaList = generalCatalogFilter
                                            .Where(x => x.PlantId == productionOrder.PlantId
                                            && x.ProductId == productionOrder.ProductId
                                            && x.TankId == productionOrder.TankId && !string.IsNullOrEmpty(x.BayArea))
                                            .Select(x => new BayAreaItem
                                            {
                                                Index = bayAreaIndex++,
                                                BayArea = x.BayArea,
        ...
        public async Task<IActionResult> GetPipeFillingControls(int id)
        {
            ...
            {
                var conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(id);
                model = await _conditioningOrderService.GetByIdAsync(id, conditioningOrder, true, userInfo);
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> SaveInitOA(int Id)
        {
            ...
            if (Id > 0)
            {
                ...
                var info = await _conditioningOrderRepository.FirstOrDefaultAsync(x => x.ProductionOrderId == Id);
                //if (info.Count == 0)
                if (info == null)
                {
                    ...
                    _logger.LogError("Se esta creando una OA desde el metodo SaveInitOA.");
        ...
        private async Task<JsonResult> SaveAnalyticalEquipament(ConditioningOrderViewModel model)
        {
            ...
            var entity = await _conditioningOrderRepository.FirstOrDefaultAsync(model.Id);
            ...
            {
                //TODO: Validar porque consulta ordenes de acondcionamiento, si es posible asumir que hacen referencia a la misma OA, quitar este query, y tomar la OA consultada arriba.
                //var ids = model?.AnalyticEquipmentList.Select(x => x.Id);
                //var orders = await _conditioningOrderRepository.GetAsync(x => ids.Contains(x.Id));
                foreach (var item in model.AnalyticEquipmentList)
                {
                    //var info = _conditioningOrderRepository.GetByIdAsync(item.Id);
                    //var info = orders.FirstOrDefault(x => x.Id == item.Id);
            ...
            _conditioningOrderRepository.ClearTracker();
        ...
        private async Task<JsonResult> SaveScalesFlowMeters(ConditioningOrderViewModel model)
        {
            ...
            foreach (var item in model.PipelineClearanceHistory)
            {
                ...
                if (pipelineClearanceEntity != null)
                {
                    ...
                    pipelineClearanceEntity.ConditioningOrder = entity;
            ...
            if (model.PipelineClearanceOA.InCompliance.HasValue && model.PipelineClearanceOA.InCompliance.Value)
            {
                ...
                else
                {
                    ...
                    pipeline.ConditioningOrder = entity;
        ...
        private async Task<JsonResult> SaveControlPipeFilling(ConditioningOrderViewModel model)
        {
            ...
            foreach (var item in pipeFillingViewModelsCustomers)
            {
                ...
                if (info != null)
                {
                    ...
                    info.TourNumber = item.TourNumber;
                    info.Name = string.IsNullOrEmpty(info.Name) && !string.IsNullOrEmpty(item.Name) ? item.Name : info.Name;
                    ...
                    _logger.LogInformation("PipeFillingCustomers from SaveControlPipeFilling " + info.Tank + "|" + item.Name + "|" + item.DeliveryNumber + "|" + "tournumber" + item.TourNumber + "|" + "ConditioningOrderId " + model.Id);
                }
                else
                {
        ...
        private async Task<JsonResult> SaveRelease(ConditioningOrderViewModel model)
        {
            try
            {
                ...
                {
                    if (entity.IsReleased.HasValue && !entity.IsReleased.Value)
                        return Json(new { Result = "BadRequest", Message = "La Orden de Producción está en proceso." });
                    ...
                    entity.CloseOrderUserName = null;
                    entity.CloseOrderUserDate = null;
                    entity.CloseOrderUserNote = null;
                }
                else if (model.IsReleased.HasValue && model.IsReleased.Value)
                {
                    if (entity.IsReleased.HasValue && entity.IsReleased.Value)
                        return Json(new { Result = "BadRequest", Message = "La Orden de Producción ya se encuentra liberada." });
                    var difSignature = Convert.ToDateTime(_config["DayChangeNewSignature"]);
                    var flagChange = entity.CreatedDate >= difSignature;
                    if (flagChange)
                        entity.State = ConditioningOrderStatus.Released.Value;
                    else
                        entity.State = ConditioningOrderStatus.ReleasedOld.Value;
                    entity.IsReleased = true;
                    entity.StepSaved = 6;
                    entity.StepSavedDescription = "Cierre del expediente de lote";
                    entity.CloseOrderUserNote = string.IsNullOrEmpty(entity.CloseOrderUserNote?.Trim()) ?
                        (string.IsNullOrEmpty(model.CloseOrderUserNote?.Trim()) ? "NA" : model.CloseOrderUserNote.Trim())
                        : entity.CloseOrderUserNote;
                ...
                if (model.ReleasedNotes != null)
                {
                    historyNotes.Add(new HistoryNotes
                    {
                        ...
                        Source = "Tabla 7: Cierre de expediente de lote",
                        User = userInfo,
                        Date = DateTime.Now
                    });
                    await AddNotes(historyNotes);
        ...
        [HttpGet]
        public async Task<IActionResult> ExportToPdf(int IdOP, int Id,
            [FromServices] ILeyendsCertificateHistoryRepository leyendsCertificateHistoryRepository,
            [FromServices] ILeyendsFooterCertificateHistoryRepository leyendsFooterCertificateHistoryRepository, int preview = 0)
        {
            ...
            try
            {
                ...
                if (IdOP > 0 && Id == 0)
                {
                    //model = await _conditioningOrderService.GetByProductionOrderIdAsync(IdOP, preview: preview, true);
                    conditioningOrder = await _conditioningOrderRepository.FirstOrDefaultAsync(x => x.ProductionOrderId == IdOP);
                }
                else if (Id > 0)
                {
                ...
                model = await _conditioningOrderService.GetByIdAsync(Id, conditioningOrder, preview: preview, style: true);
                //model = await _exportPDFService.GetModel(IdOP, Id, true, preview);//  
                ...
                foreach (var item in model.LayoutLeyends)
                {
                    item.Text = item.Text.Replace("{CaseTag:Location}", model.Plant)
                                              ...
                                              .Replace("{CaseTag:ProductCode}", model.ProductName)
                                              .Replace("{CaseTag:SelectedTankFilter}", model.Tank);
                    //item.Text = item.Text.Replace("<p style=\"text-align: justify;\">dsds</p>", "");
                    //item.Text = item.Text.Replace("<p style=\"text-align: justify;\">&nbsp;</p>", "");
                }
                ...
                if (chkGeneralList.Count > 0 && preview.Equals(0))
                {
                    for (int i = 0; i < chkGeneralList.Count; i++)
                    {
                        var t = await _exportPDFService.GetRptChkList(chkGeneralList[i], conditioningOrder);
                        rptChkList.Add(t);
                    }
                ...
                XRLabel TitleZero = _rptOA.FindControl("label0", true) as XRLabel;
                TitleZero.Text = model.LayoutLeyends.Where(x => x.Step.Equals(0)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendZero = _rptOA.FindControl("xrTextZero", true) as XRRichText;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(0)).Select(x => x.Text).FirstOrDefault();
                LeyendZero.Html = leyend;
                leyend = string.Empty;
                XRLabel TitleOne = _rptOA.FindControl("label4", true) as XRLabel;
                TitleOne.Text = model.LayoutLeyends.Where(x => x.Step.Equals(1)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendOne = _rptOA.FindControl("xrTextOne", true) as XRRichText;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(1)).Select(x => x.Text).FirstOrDefault();
                LeyendOne.Html = leyend;

                leyend = string.Empty;

                var difSignature = Convert.ToDateTime(_config["DayChangeNewSignature"]);
                var flagChange = model.CreatedDate.Value >= difSignature;
                ///of tr signature for olders OA
                if (!flagChange)
                {
                    var xrTable = tabla7.FindControl("xrTable1", true) as XRTable;
                    xrTable.Rows.FirstRow.Cells.RemoveAt(0);
                }
                XRLabel TitleTwo = _rptOA.FindControl("label17", true) as XRLabel;
                TitleTwo.Text = model.LayoutLeyends.Where(x => x.Step.Equals(2)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendTwo = _rptOA.FindControl("xrTextTwo", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(2)).Select(x => x.Text).FirstOrDefault();
                LeyendTwo.Html = leyend;

                XRLabel TitleThree = _rptOA.FindControl("label25", true) as XRLabel;
                TitleThree.Text = model.LayoutLeyends.Where(x => x.Step.Equals(3)).Select(x => x.Title).FirstOrDefault();

                XRRichText LeyendThree = _rptOA.FindControl("xrTextThree", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(3)).Select(x => x.Text).FirstOrDefault();
                LeyendThree.Html = leyend;

                XRLabel TitleFor = _rptOA.FindControl("label49", true) as XRLabel;
                TitleFor.Text = model.LayoutLeyends.Where(x => x.Step.Equals(4)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendFor = _rptOA.FindControl("label50", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(4)).Select(x => x.Text).FirstOrDefault();
                LeyendFor.Html = leyend;

                //EQUIPOS Y ACCESOSRIOS
                //XRRichText Teams = _rptOA.FindControl("xrTeams", true) as XRRichText;
                //leyend = string.Empty;
                //leyend = model.LayoutLeyends.Where(x => x.Step.Equals(2)).Select(x => x.Text).FirstOrDefault();
                //Teams.Html = leyend;

                XRLabel TitleFive = tabla6.FindControl("label4", true) as XRLabel;
                TitleFive.Text = model.LayoutLeyends.Where(x => x.Step.Equals(5)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendFive = tabla6.FindControl("label32", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(5)).Select(x => x.Text).FirstOrDefault();
                LeyendFive.Html = leyend;

                XRLabel TitleSix = tabla6.FindControl("label1", true) as XRLabel;
                TitleSix.Text = model.LayoutLeyends.Where(x => x.Step.Equals(6)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendSix = tabla6.FindControl("label5", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(6)).Select(x => x.Text).FirstOrDefault();
                LeyendSix.Html = leyend;

                XRLabel TitleSeven = tabla6.FindControl("label3", true) as XRLabel;
                TitleSeven.Text = model.LayoutLeyends.Where(x => x.Step.Equals(7)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendSeven = tabla6.FindControl("xrRichText1", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(7)).Select(x => x.Text).FirstOrDefault();
                LeyendSeven.Html = leyend;

                XRLabel TitleEight = tabla7.FindControl("label1", true) as XRLabel;
                TitleEight.Text = model.LayoutLeyends.Where(x => x.Step.Equals(8)).Select(x => x.Title).FirstOrDefault();
                XRRichText LeyendEight = tabla7.FindControl("label32", true) as XRRichText;
                leyend = string.Empty;
                leyend = model.LayoutLeyends.Where(x => x.Step.Equals(8)).Select(x => x.Text).FirstOrDefault();
                LeyendEight.Html = leyend;
                ...
                var fileNames = model.PipeFillingControl.SelectMany(x => x.PipesList)
                    ...
                    .Select(x => x.PathFile)
                    .ToList();
                if (fileNames.Any() && preview.Equals(0))
                {
        ...
        public async Task<IActionResult> SaveCloseOpen(ConditioningOrderViewModel model)
        {
            ...
            try
            {
                ...
                entity = await _conditioningOrderRepository.FirstOrDefaultAsync(model.Id);
                var entityClone = new ConditioningOrder();
                entityClone = (ConditioningOrder)entity.Clone();
                if (entity == null)
                {
        ...
        public async Task<IActionResult> CheckListTourNumber(int Id)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { Result = "fail", Message = "Ocurrio un error en CheckListTourNumber." });
            }
        ...
        public async Task<IActionResult> SaveTourNumberPipeNumberRel([FromForm] ConditioningOrderViewModel model)
        {
            ...
            catch (Exception ex)
            {
                ...
                return Json(new { Result = "fail", Message = "Ocurrio un error en SaveTourNumberPipeNumberRel." });
            }
            return Json(new { Result = "Ok", Message = "CheckList Guardado correctamente" });
        }
        ...
        [HttpGet]
        public async Task<IActionResult> ExportChecklistPdf(int IdOP, int Id, int checkListId, string distributionBatch, int option, int preview = 0)
        {
            using MemoryStream ms = new MemoryStream();
            try
            {
                ...
                if (conditioningOrder is null)
                    return BadRequest();
                if (conditioningOrder != null)
                    IdOP = conditioningOrder.ProductionOrderId;
                ...
                List<ConditioningOrderViewModel> rptDataSource = new List<ConditioningOrderViewModel>();
                var model = await _exportPDFService.GetModel(IdOP, Id, conditioningOrder, preview: preview);//  
                if (model == null)
                {
                    return BadRequest();
                }
                ...
                if (chkGeneralList.Count > 0)
                {
                    for (int i = 0; i < chkGeneralList.Count; i++)
                    {
                        if (option == 2)
                        {
                            var t = await _exportPDFService.GetRptChkList(chkGeneralList[i], conditioningOrder, preview);
                            rptChkList.Add(t);
                        }
                        else
                        {
                            var t = await _exportPDFService.GetRptChkListQuestionsOne(chkGeneralList[i], conditioningOrder);
                            rptChkList.Add(t);
                        }
                    }
        ...
        [HttpGet]
        [Authorize(SecurityConstants.EDITAR_OA)]
        public async Task<ActionResult> GetCustomerName(string tourNumber, string tankNumber, string deliveryNumber)
        {
            var CustomerName = string.Empty;
            try
            {
                var customerBD = await _conditioningOrderService.GetCustomerName(tourNumber, tankNumber, deliveryNumber);
            ...
            catch (Exception ex)
            {
                //this._logger.LogError(ex.Message);
                this._logger.LogError(ex, "Error.");
                return Json(new { Result = "Error", Message = "Surgio un error al obtener nombre GetCustomerName." });
            }
            return Json(new { Result = "Ok", Message = CustomerName });
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<JsonResult> CloseOrderUser([FromForm] ConditioningOrderViewModel model)
        {
            List<HistoryNotes> historyNotes = new List<HistoryNotes>();
            try
            {
                ...
                var entity = await _conditioningOrderRepository.FirstOrDefaultAsync(model.Id);
                var userInfo = await GetUserId(HttpContext.User.Claims.ToList());
                entity.CloseOrderUserName = model.CloseOrderUserName;
                entity.CloseOrderUserDate = model.CloseOrderUserDate;
                entity.CloseOrderUserNote = model.CloseOrderUserNote;
                await _conditioningOrderRepository.UpdateAsync(entity);
                if (model.CloseOrderUserNote != null)
                {
                    historyNotes.Add(new HistoryNotes
                    {
                        ProductionOrderId = entity.ProductionOrderId,
                        Note = string.IsNullOrEmpty(model.CloseOrderUserNote) ? "NA" : model.CloseOrderUserNote,
                        Source = "Tabla 7: Cierre de Orden de Acondicionamiento",
                        User = userInfo,
                        Date = DateTime.Now
                    });
                    await AddNotes(historyNotes);
            ...
            return Json(new { Result = "Ok", Message = "Se firmó correctamente." });
        }
    }
}