...
namespace LiberacionProductoWeb.Services
{
    public class ReportAuditTrailService : IReportAuditTrailService
    {
            ...
            var ReportDB = from Report in await _reportAuditTrailRepository.GetAllAsync()
                           select new ReportAuditTrailViewModel
                           {
                               ...
                               PreviousVal = Report.Detail.Contains("Fecha") ? ConvertDateFormat(Report.PreviousValue).Result :
                                              Report.Detail.Contains("Planta") ? GetPlantName(Report.PreviousValue, Report.User, plantsDb).Result : RemoveHtml(Report.PreviousValue).Result,
                               NewVal = Report.Detail.Contains("Fecha") ? ConvertDateFormat(Report.NewValue).Result :
                                              Report.Detail.Contains("Planta") ? GetPlantName(Report.NewValue, Report.User, plantsDb).Result : RemoveHtml(Report.NewValue).Result,
        ...
        async Task<String> RemoveHtml(String html)
        {
            ...
            {
                dom.LoadHtml(html);
                resultado = HttpUtility.HtmlDecode(dom.DocumentNode.InnerText);
            }
            else
            {
                resultado = html;
