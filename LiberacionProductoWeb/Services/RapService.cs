...
namespace LiberacionProductoWeb.Services
{
    public class RapService : IRapService
    {
        ...
        public RapService(IPlantasRepository plantasRepository,
            IProductoRepository productoRepository,
            IConfiguration config,
            IFormulaCatalogRepository formulaCatalog,
        ...
        public async Task<List<PipaModel>> GetRapPipas(string plantId, string productId, string tankId)
        {
            ...
            //TODO: Validar consulta para mejorar performance
            var ids = opList.Select(x => x.Id).Distinct();
            var orders = await _conditioningOrderRepository.GetAsync(x => ids.Contains(x.ProductionOrderId));
            foreach (var opItem in opList)
            {
                ...
                var oaItem = orders.FirstOrDefault(x => x.ProductionOrderId == opItem.Id);
        ...
        public async Task<List<RapComplemento>> GetRapCompPipas(string plantId, string productId, string tankId)
        {
            ...
            foreach (var opItem in opList)
            {
                ...
                if (oaItem != null)
                {
                    var pipeAnalysis = await _pipeFillingAnalysisRepository.GetAsync(x => x.ConditioningOrderId == oaItem.Id);
                    var pipeControl = await _pipeFillingControlRepository.GetAsync(x => x.ConditioningOrderId == oaItem.Id);
                    var pipeComplement = await _complementoPipaRepository.GetAsync(x => x.ConditioningOrderId == oaItem.Id);

                    foreach (var pipeControlItem in pipeControl)
                    {
                        var pipeFilling = (await _pipeFillingRepository.GetAsync(x => x.PipeFillingControlId == pipeControlItem.Id)).FirstOrDefault();
                        var pipeComplementTemp = pipeComplement.FirstOrDefault(x => x.NumeroLoteDistribucion == pipeFilling.DistributionBatch);
                        if (pipeFilling != null)
                        {
                            rapPipas.Add(new RapComplemento
                            {
                                ...
                                NumeroLoteProduccion = opBatch?.Number,
                                NumeroLoteDistribucion = pipeFilling.DistributionBatch,
                                DisposicionPNC = pipeComplementTemp?.DisposicionPNC ?? "NA",
                                DisposicionPNCText = dispositions.Where(x => x.Value == pipeComplementTemp?.DisposicionPNC).Any() ?
                                dispositions.Where(x => x.Value == pipeComplementTemp?.DisposicionPNC).Select(x => x.Text).FirstOrDefault() : "NA",
                                FolioControlCambios = pipeComplementTemp?.FolioControlCambios ?? "NA",
                                FolioEventoAdverso = pipeComplementTemp?.FolioEventoAdverso ?? "NA",
                                FolioPNC = pipeFilling.ReportPNCFolio ?? "NA",
                                FolioRetiroProductos = pipeComplementTemp?.FolioRetiroProductos ?? "NA",
                                FolioTrabajoNoConforme = pipeComplementTemp?.FolioTrabajoNoConforme ?? "NA",
                                NumeroDeDevolucion = pipeComplementTemp?.NumeroDeDevolucion ?? "NA",
                                Observaciones = pipeComplementTemp?.Observaciones ?? "NA",
                                InformeMayor = string.IsNullOrEmpty(pipeComplementTemp?.InformeMayor?.Trim()) ? "NA" : pipeComplementTemp.InformeMayor.Trim(),
                                InformeMenor = string.IsNullOrEmpty(pipeComplementTemp?.InformeMenor?.Trim()) ? "NA" : pipeComplementTemp.InformeMenor.Trim()
                            });
                        }
                    }
                }
        ...
        public async Task<List<RapComplemento>> GetRapCompTanques(string plantId, string productId, string tankId)
        {
            ...
            foreach (var op in opList)
            {
                ...
                rapTanques.Add(new RapComplemento
                {
                    ...
                    InformeMayor = string.IsNullOrEmpty(tankComplement?.InformeMayor?.Trim()) ? "NA" : tankComplement.InformeMayor.Trim(),
                    InformeMenor = string.IsNullOrEmpty(tankComplement?.InformeMenor?.Trim()) ? "NA" : tankComplement.InformeMenor.Trim(),
                });
            };
