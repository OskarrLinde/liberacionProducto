...
namespace LiberacionProductoWeb.Services
{
    public class ExportPDFService : Controller, IExportPDFService
    {
        ...
        public ExportPDFService(ILogger<ExportPDFService> logger,
        ...
        public async Task<ProductionOrderViewModel> GetOrderProductionModelById(int id, int preview = 0)
        {
            ...
            else
            {
                ...
                model.PlantId = await GetIdentificador(entity.PlantId);
        ...
        public async Task<ConditioningOrderViewModel> GetModel(int IdOP, int Id, ConditioningOrder conditioningOrder, bool style = false, int preview = 0)
        {
            var model = new ConditioningOrderViewModel();

            //if (IdOP > 0 && Id == 0)
            //{
            //model = await _conditioningOrderService.GetByProductionOrderIdAsync(IdOP, preview: preview, style: style);
            //}
            //else if (Id > 0)
            //{
        ...
        public async Task<RptChkList> GetRptChkList(CheckListGeneralViewModel chkGeneral, ConditioningOrder conditioningOrder, int preview = 0)
        {
            ...
            var labelsDb = await _principalService.CheckListRecordLabels(idOA, checkListId, conditioningOrder.ProductionOrderId);

            model.Localizate = labelsDb.Localizate;
            model.Product = labelsDb.Product;
            model.Pipe = labelsDb.Pipe;

            model = await GetRptCheckListVMs(model, conditioningOrder, preview);
            ...
            ////titles xrRichTexLeyendOne
            XRLabel titleOne = rptCheckList.FindControl("xrLabelTitleOne", true) as XRLabel;
            titleOne.Text = model.TitleOne;
            XRRichText Leyend = rptCheckList.FindControl("xrRichTexLeyendOne", true) as XRRichText;
            Leyend.Html = model.LeyendOne;
            XRLabel titleTwo = rptCheckList.FindControl("xrLabelTitleTwo", true) as XRLabel;
            titleTwo.Text = model.TitleTwo;
            XRRichText LeyendTwo = rptCheckList.FindControl("xrRichTexLeyendTwo", true) as XRRichText;
            LeyendTwo.Html = model.LeyendTwo;
            rptCheckList.CreateDocument();


            return rptCheckList;
        }
        public async Task<CheckListVM> GetRptCheckListVMs(CheckListVM model, ConditioningOrder conditioningOrder, int preview = 0)
        {
        ...
        //sin llamadas a OA en nivel superior.
        ...
        public async Task<RptChkList> GetRptChkListQuestionsOne(CheckListGeneralViewModel chkGeneral, ConditioningOrder conditioningOrder)
        {
            List<RptChkList> retList = new List<RptChkList>();
            List<CheckListVM> lstCheckListVM = new List<CheckListVM>();
            ...
            model = await GetRptCheckListVMsStepOne(model, conditioningOrder);
        ...
        public async Task<CheckListVM> GetRptCheckListVMsStepOne(CheckListVM model, ConditioningOrder conditioningOrder)
        {
            CheckListPipeRecordAnswer modelrecord = new CheckListPipeRecordAnswer();
            var userInfo = await _userManager.FindByNameAsync(_httpContextAccessor.HttpContext.User.Identity.Name);
            CheckListPipeDictiumAnswer dictiumAnswer = new CheckListPipeDictiumAnswer();
            try
            {
                ...
                model.LeyendTwo = questionsDefault.Where(x => x.Step.Equals(CheckListSteps.LeyendTwo.Value)).Select(x => x.Requirement).FirstOrDefault();
                //var DbCatAnswer = new List<CheckListPipeAnswer>();
                model.DistributionBatch = string.IsNullOrEmpty(model.DistributionBatch) ? null : model.DistributionBatch;

                var DbCatAnswer = await _checkListPipeAnswerRepository.
                                    GetAsync(x => x.NumOA == model.NumberOrder
                                    && x.CheckListPipeDictiumId == model.checkListId && x.Group == "IV" && x.DistributionBatch == model.DistributionBatch);
        ...
        private async Task<string> GetIdentificador(string plantId)
        {
            var plant = string.Empty;
            plant = (await _principalService.GetIdentificadorPlant(plantId));
            return plant;
        }
        #endregion
