...
namespace LiberacionProductoWeb
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            var cadenadeconexion = Configuration.GetConnectionString("DefaultConnection");
            var cadenadeconexionEnc = Seguridad.Encription.DesEncripta(cadenadeconexion);
            //var cadenadeconexionEnc = cadenadeconexion;

            services.AddDbContext<AppDbContext>(options => options.UseSqlServer(cadenadeconexionEnc,
                                                            sqlServerOptions => sqlServerOptions.CommandTimeout(180)), ServiceLifetime.Transient);

            var cadenadeconexion01 = Configuration["LProdConnection:ServiceApiKey"];
            var cadenadeconexionEnc01 = Seguridad.Encription.DesEncripta(cadenadeconexion01);
            //var cadenadeconexionEnc01 = cadenadeconexion01;

            services.AddDbContext<AppDbExternalContext>(options => options.UseSqlServer(cadenadeconexionEnc01,
                                                            sqlServerOptions => sqlServerOptions.CommandTimeout(180)), ServiceLifetime.Transient);
            ...
            //services.Configure<AccessOptions>(Configuration.GetSection("AccessOptions"));
            ...
            var passW = emailConfig.Password;
            emailConfig.Password = Seguridad.Encription.DesEncripta(passW);
            //emailConfig.Password = passW;
            ...
            services.AddScoped<ILeyendsTextRepository, LeyendsTextRepository>();
            services.AddScoped<ILayoutLeyendTextService, LayoutLeyendTextService>();
            services.AddScoped<ILeyendsPreviewTextRepository, LeyendsPreviewTextRepository>();
            services.AddScoped<ILeyendsTextHistoryRepository, LeyendsTextHistoryRepository>();
        ...
        private void ConfigureAuthorizationPolicies(IServiceCollection services)
        {
            services.AddAuthorization(options =>
            {
                ...
                options.AddPolicy(SecurityConstants.CONSULTAR_LAYOUT_ORDEN_PRODUCCION_CONDITIONING,
                    policy => policy.RequireClaim(SecurityConstants.PERMISSION, SecurityConstants.CONSULTAR_LAYOUT_ORDEN_PRODUCCION_CONDITIONING));

                options.AddPolicy(SecurityConstants.EDITAR_LAYOUT_ORDEN_PRODUCCION_CONDITIONING,
                   policy => policy.RequireClaim(SecurityConstants.PERMISSION, SecurityConstants.EDITAR_LAYOUT_ORDEN_PRODUCCION_CONDITIONING));
                ...
                options.AddPolicy(SecurityConstants.CONSULTAR_LAYOUT_CHECKLIST,
                   policy => policy.RequireClaim(SecurityConstants.PERMISSION, SecurityConstants.CONSULTAR_LAYOUT_CHECKLIST));

                options.AddPolicy(SecurityConstants.EDITAR_LAYOUT_CHECKLIST,
                    policy => policy.RequireClaim(SecurityConstants.PERMISSION, SecurityConstants.EDITAR_LAYOUT_CHECKLIST));
            });
        }
        ...
        private async Task CreateStatus(IStateRepository statusRepository)
        {
            ...
            status.Add(new States { Description = "Expediente de lote cerrado", CreationDate = DateTime.Now, UpdateDate = DateTime.Now, Active = true, Group = "OA" });
